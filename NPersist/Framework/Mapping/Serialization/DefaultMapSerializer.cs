...
namespace Puzzle.NPersist.Framework.Mapping.Serialization
{
	public class DefaultMapSerializer : MapSerializerBase
	{
		...
		protected virtual string SerializePropertyMap(IPropertyMap propertyMap)
		{
			...
			if (propertyMap.DOLFindByGroup >= 0)
			{
				xml.Append(" DOLFindByGroup=\"" + propertyMap.DOLFindByGroup + "\"");
			}
			if (propertyMap.DOLFindByGroupIndex >= 0)
			{
				xml.Append(" DOLFindByGroupIndex=\"" + propertyMap.DOLFindByGroupIndex + "\"");
			}
